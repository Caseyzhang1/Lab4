...
This variable's distribution is normal because none of the p-values are below the threshold (.05) so it's insignificant.
...
No, the p-value is .023, which is less than .05. This is significant therefore there is no equal variance.
...
The data was skewed at first, so we conducted a logarithmic transformation on the data. After the transformation, the outcome was that the data became normally distributed. We also changed the bin width of the graph, after which our graph became visually representative of a normal distribution.
